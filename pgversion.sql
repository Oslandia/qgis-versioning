...
-- Die grundlegenden Geometrieparameter des Ausgangslayers ermitteln
    ...
    execute 'delete from versions.version_tables 
               where version_table_schema = '''||mySchema||'''
                 ...
                 and version_view_schema = '''||mySchema||'''
                 and version_view_name = '''||myTable||'_version''
                 and version_view_geometry_column = '''||geomCol||''';';
...
$BODY$    
  ...
  BEGIN	
    ...
    logViewQry := 'select distinct revision, to_timestamp(systime/1000), project,  logmsg
                           from  '||versionLogTable||'
                           where commit 
...
-- Das erste Komma  aus dem String entfernen
     ...
     deleteQry := 'insert into versions.'||mySchema||'_'||myTable||'_version_log ('||fields||', action) 
      ...
      insertQry := 'insert into versions.'||mySchema||'_'||myTable||'_version_log ('||fields||', action) 
