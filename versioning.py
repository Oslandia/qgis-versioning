...
class Versioning:
    ...
    def update(self):
        ...
        for name,layer in versionned_layers.iteritems():
            ...
            scur = scon.cursor()
            table = uri.table()[:-5] # remove suffix _view
            scur.execute("SELECT rev, branch, table_schema, conn_info, max_hid "+
                "FROM initial_revision "+
                "WHERE table_name = '"+table+"'")
            [rev, branch, table_schema, conn_info, current_max_hid] = scur.fetchone()
    ...
    def checkout(self):
        ...
        for name,layer in versionned_layers.iteritems():
            ...
            cur.execute("SELECT MAX(hid) FROM "+schema+"."+table)
            max_pg_hid = int(cur.fetchone()[0])
            ...
            if not filename:
                ...
                if not cur.fetchone(): # no record, insert
                    cur.execute("INSERT INTO initial_revision(rev, branch, table_schema, table_name, conn_info, max_hid) VALUES ("+str(current_rev)+", '"+branch+"', '"+schema+"', '"+table+"', '"+escapeQuotes(uri.connectionInfo())+"', "+str(max_pg_hid)+")" )
                con.commit()
                con.close()
            ...
            sql =("CREATE TRIGGER update_"+table+" INSTEAD OF UPDATE ON "+table+"_view\n"+
                  "BEGIN\n"+
                    ...
                    "UPDATE "+table+" SET "+hcols['rev_end']+" = (SELECT rev FROM initial_revision WHERE table_name = '"+table+"'), "+hcols['child']+" = "+max_fid_sub+" WHERE OGC_FID = old.OGC_FID;\n"+
                  "END")
            print sql
            cur.execute(sql)  
